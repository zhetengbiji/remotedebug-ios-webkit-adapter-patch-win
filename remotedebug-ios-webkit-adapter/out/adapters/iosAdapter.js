...
class IOSAdapter extends adapterCollection_1.AdapterCollection {
    ...
    getTargets() {
        ...
        logger_1.Logger.log('iosAdapter.getTargets');
            ...
            devices.forEach(d => {
                ...
                if (d.deviceId === 'SIMULATOR') {
                    ...
                    getter = this.getDeviceVersion(d.deviceId).then(v => {
                        ...
                        logger_1.Logger.log(`error.iosAdapter.getTargets.getDeviceVersion.failed.fallback, device=${d}`);
                        d.version = '9.3.0';
                        return Promise.resolve(d);
                    });
                }
    ...
    static getProxyPath() {
        ...
        return new Promise((resolve, reject) => {
            if (os.platform() === 'win32') {
                const proxy = path.resolve(__dirname, '../../node_modules/vs-libimobile/ios-webkit-debug-proxy/1.8/ios_webkit_debug_proxy.exe');
                try {
                    fs.statSync(proxy);
                    resolve(proxy);
                }
    ...
    static getDeviceInfoPath() {
        logger_1.debug(`iOSAdapter.getDeviceInfoPath`);
        return new Promise((resolve, reject) => {
            if (os.platform() === 'win32') {
                const proxy = path.resolve(__dirname, '../../node_modules/vs-libimobile/libimobiledevice-windows-master/ideviceinfo.exe');
                try {
                    fs.statSync(proxy);
                    resolve(proxy);
                }
    ...
    getDeviceVersion(uuid) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.debug(`iOSAdapter.getDeviceVersion`);
            const _iDeviceInfoPath = yield IOSAdapter.getDeviceInfoPath();
            var proc = yield child_process_promise_1.execFile(_iDeviceInfoPath, ['-u', `${uuid}`, '-k', 'ProductVersion']);
            let deviceVersion = '';
            if (!proc.err) {
